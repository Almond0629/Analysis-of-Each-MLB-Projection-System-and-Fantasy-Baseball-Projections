...
def create_projections(df, year=2026):
    ...
    atc_w = series_by_name(atc_df, 'W%')
    tb_w = series_by_name(tb_df, 'W%')

    atc_l = series_by_name(atc_df, 'L%')
    tb_l = series_by_name(tb_df, 'L%')

    atc_sv = series_by_name(atc_df, 'SV%')
    tb_sv = series_by_name(tb_df, 'SV%')
    oopsy_sv = series_by_name(oopsy_df, 'SV%')

    atc_hld = series_by_name(atc_df, 'HLD%')
    tb_hld = series_by_name(tb_df, 'HLD%')
    oopsy_hld = series_by_name(oopsy_df, 'HLD%')

    atc_bs = series_by_name(atc_df, 'BS%')

    tb_h = series_by_name(tb_df, 'H%')
    oopsy_h = series_by_name(oopsy_df, 'H%')

    tb_r = series_by_name(tb_df, 'R%')
    oopsy_r = series_by_name(oopsy_df, 'R%')

    tb_er = series_by_name(tb_df, 'ER%')
    oopsy_er = series_by_name(oopsy_df, 'ER%')

    tb_hr = series_by_name(tb_df, 'HR%')
    zips_dc_hr = series_by_name(zips_dc_df, 'HR%')
    oopsy_hr = series_by_name(oopsy_df, 'HR%')

    atc_ibb = series_by_name(atc_df, 'IBB%')
    tb_ibb = series_by_name(tb_df, 'IBB%')
    steamer_ibb = series_by_name(steamer_df, 'IBB%')

    tb_hbp = series_by_name(tb_df, 'HBP%')
    oopsy_hbp = series_by_name(oopsy_df, 'HBP%')

    atc_k = series_by_name(atc_df, 'K%')
    tb_k = series_by_name(tb_df, 'K%')
    oopsy_k = series_by_name(oopsy_df, 'K%')

    zips_dc_bb = series_by_name(zips_dc_df, 'BB%')
    dc_bb = series_by_name(dc_df, 'BB%')
    oopsy_bb = series_by_name(oopsy_df, 'BB%')

    key_df['W%'] = key_df['Name_key'].map(atc_w) * 0.5 + key_df['Name_key'].map(tb_w) * 0.5
    key_df['L%'] = key_df['Name_key'].map(atc_l) * 0.5 + key_df['Name_key'].map(tb_l) * 0.5
    key_df['SV%'] = key_df['Name_key'].map(atc_sv) * 0.333 + key_df['Name_key'].map(tb_sv) * 0.333 + key_df['Name_key'].map(oopsy_sv) * 0.333
    key_df['HLD%'] = key_df['Name_key'].map(atc_hld) * 0.6 + key_df['Name_key'].map(tb_hld) * 0.2 + key_df['Name_key'].map(oopsy_hld) * 0.2
    key_df['BS%'] = key_df['Name_key'].map(atc_bs)
    key_df['H%'] = key_df['Name_key'].map(tb_h) * 0.5 + key_df['Name_key'].map(oopsy_h) * 0.5
    key_df['R%'] = key_df['Name_key'].map(tb_r) * 0.5 + key_df['Name_key'].map(oopsy_r) * 0.5
    key_df['ER%'] = key_df['Name_key'].map(tb_er) * 0.5 + key_df['Name_key'].map(oopsy_er) * 0.5
    key_df['HR%'] = key_df['Name_key'].map(tb_hr) * 0.5 + key_df['Name_key'].map(zips_dc_hr) * 0.2 + key_df['Name_key'].map(oopsy_hr) * 0.3
    key_df['IBB%'] = key_df['Name_key'].map(atc_ibb) * 0.5 + key_df['Name_key'].map(tb_ibb) * 0.3 + key_df['Name_key'].map(steamer_ibb) * 0.2
    key_df['HBP%'] = key_df['Name_key'].map(tb_hbp) * 0.5 + key_df['Name_key'].map(oopsy_hbp) * 0.5
    key_df['K%'] = key_df['Name_key'].map(atc_k) * 0.333 + key_df['Name_key'].map(tb_k) * 0.333 + key_df['Name_key'].map(oopsy_k) * 0.333
    key_df['BB%'] = key_df['Name_key'].map(zips_dc_bb) * 0.333 + key_df['Name_key'].map(dc_bb) * 0.333 + key_df['Name_key'].map(oopsy_bb) * 0.333
...
# player_list = player_list[player_list['Position'].isin(['RP'])].copy()
...
#         print(name)
# rel_df.to_csv(f'./Relievers/Fantasy/2026/2026_Fantasy_Relievers.csv', index=False)

rel_df = get_g_ip_tbf_from_atc_for_fantasy_names(relievers_xlsx_path='/Users/almond/Desktop/Baseball/Projection_model/Relievers/Fantasy/2026/2026_Fantasy_Relievers.xlsx',
    atc_csv_path='/Users/almond/Desktop/Baseball/Projection_model/Relievers/ATC/2026_ATC_Projections.csv')
